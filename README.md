...
**Detecting errors when testing or building:** Build errors appear in **View → Output** with **Show output from: Build**. When you run with F5, switch the Output window to **Show output from: Debug** to see all runtime messages (startup, discovery, save, SDK errors). If you run without the debugger, open **log.txt** in the same folder as the executable (e.g. `src\App\bin\x86\Debug\net10.0-windows\log.txt`). See **[docs/FITTING_DEVELOPER_NOTE.md](docs/FITTING_DEVELOPER_NOTE.md)** for the full table and an **"Interpreting your debug output"** section that explains lines like `TargetInvocationException`, `E_UNCONFIGURED_DEVICE`, `Serial=-1` (unprogrammed device), and `E_SEND_FAILURE` (e.g. Right port). That note also describes the current memory-aware read/write and session-end save flow (ReadMemorySnapshotAsync, WriteMemorySnapshotAsync, RunSaveAllOnDispatcherAsync, per-memory dirty tracking). A first-chance TargetInvocationException during "Creating product" may appear in Debug output; it is often caught internally by the SDK—if the app continues and loads parameters normally, it can be ignored. Our code unwraps and logs the inner exception when we catch it (LibraryService, SdkManager, SettingsEnumerator). During save (ApplySnapshotValuesToSdkParameters), if a parameter setter throws, TrySetParameterValue logs `[SettingsEnumerator] TrySetParameterValue TargetInvocationException id=... inner=...` so the real SDK error is visible in Debug output.
...
**Connect Devices UX:** The Connect Devices screen uses a two-panel layout: **Setup** (left) — Product Library dropdown, **Step 2: Programmer** (one or more selectable programmer cards when multiple are found); **Discover & Connect** (right) — **Scan for Hearing Aids** button, Rescan/Stop, then a list of **detected devices only** (cards with checkbox, serial/firmware, Product ID, Chip ID, Status, Parameter Lock when applicable, Battery Voltage when readable). **Programmer cards** show: name (HI-PRO), S/N, Firmware, Backend (D2XX/Wireless), and optional Description. Multiple programmers/interfaces are shown as selectable cards when available. **Hearing-aid cards** show: side + model, Serial, Firmware, Product ID, Chip ID, Status (Detected/Connected), **"Device reports parameter lock (write may still succeed)"** when IDeviceInfo.ParameterLockState is true (tooltip clarifies this is device-reported at connect; some devices allow write despite it; save will attempt write and verify), and **Battery: X.XX V** when Product.BatteryAverageVoltage is readable after connection. Battery percentage is shown only when the SDK provides it; otherwise battery voltage is shown when available. A **status bar** under the title shows programmer state (e.g. "HI-PRO Found | USB OK | Driver OK") with Scan Wired, Scan Wireless, Help, and Cancel. Footer: **Selected: Left (1)** summary and green **Connect** button (disabled when none selected; shows "Connecting…" while connecting). After a successful connection, the app **automatically navigates to the Fitting** screen; the **top header** (app shell only) shows "Connected" (green) and the connected device name. **Navigation** (Audiogram, Fitting, Session Summary) is enabled only when connected; **AppSessionState** holds connection state, display name, and per-ear identity. **Connection status is shown only in the global top header**—no duplicate status block on the Fitting page.
...
- **Save to Device & End** — Stops live mode, runs **Save ALL Memories to NVM** (dirty memories by default) for each connected hearing aid via **Burn to NVM** + verify per memory, then **waits 500 ms** after the last burn before disconnecting. A **waiting dialog** ("Saving to device… Please wait.") is shown. Save is **skipped** with a clear message if: (1) **unprogrammed devices** (Serial=-1 or 0); (2) **device not configured**. **NVM-only policy:** Only **Burn to NVM** counts as save; session-end uses `BurnMemoryToNvmAsync` and `VerifyMemoryMatchesNvmAsync` (logs `[NVM] Burn M#`, `[NVM] Verify M# OK/FAIL`). If save or verify fails, the session is not ended as success; the user sees the error and can reconnect.
...
If only one device is connected (e.g. left ear only), the dialog states that saving will apply only to the connected device(s). After **Save & End** (on success) or **End Without Saving**, the app force-disconnects, navigates back to **Connect Devices** (with connection inactive so headset selection is available), and **automatically starts a new wired discovery scan** after a short debounce (500 ms). Session-end steps are logged (e.g. `SessionEnd: StopLiveMode`, `SessionEnd: SaveToDevice (Left/Right) memory=#`, `SessionEnd: Disconnect`, `SessionEnd: RestartDiscovery` or `SessionEnd: NavigateToConnect after save failure`). SDK calls run off the UI thread; UI updates use the Dispatcher; resources are released in `finally` blocks.

**Save-Settings workflow (NVM-only):** The app follows a **strict NVM-only** persistence policy. **We do not treat "Write to RAM" as saving;** any **Save** action ends with **Burn to NVM**. Persistence across disconnect **requires writing to NVM** (e.g. `WriteParameters(memoryIndex)` or `ParameterSpace.kNvmMemoryN`), not only to RAM (`kActiveMemory`). The app discovers the .NET NVM write API at runtime via reflection (ParameterSpace and `BeginWriteParameters` overloads); if no NVM write is found, it logs **"Not supported by docs: no NVM write API found; persistence may fail after disconnect."** On connect and on memory switch, the app **Restore from NVM** then **Read from RAM** so the session reflects persisted state. Pipeline: **Connected → Configured → Synced → Dirty → Save Current Memory / Save ALL Memories → Burn to NVM → Verify → Persisted to NVM (verified)**. Fitting toolbar: **Reload from NVM** (Restore + Read for current memory), **Save Current Memory** (Burn to NVM), **Save ALL Memories** (Burn all dirty to NVM). Status line shows **Persisted to NVM (verified)** after a successful save with read-back verification. SDK semantics follow vendor sample `presuite_memory_switch.py` (Read/Write NVM, memory switching). Logs: `[NVM] Restore M#`, `[NVM] Burn M#`, `[NVM] Verify M# OK/FAIL`, `[WriteVerify]` (write/read-back diagnostics). **Write verification:** After each write, the app immediately reads back from the same memory and compares key parameters; verification pass/fail is logged and reflected in the status. **Configuration gate:** Save is disabled when not configured; see Device initialization gate below.

**Device initialization gate (single gate):** All Read/Write to the device are guarded by **DeviceInitializationService.EnsureInitializedAndConfiguredAsync(session, side, ct)**. It runs inside **SdkGate** (single SemaphoreSlim) so only one SDK operation runs at a time; it must be called from the UI/STA thread. It ensures InitializeDevice(adaptor) has completed and, if the SDK reports not configured or throws **E_UNCONFIGURED_DEVICE**, sets **IsConfigured** false and **LastConfigError**. Session state is exposed on **DeviceSessionService**: **IsDeviceConnected**, **IsInitialized**, **IsConfigured**, **LastConfigError**, **DeviceFirmwareId** / **LeftSerial** / **RightSerial** / **LeftModel** / **RightModel**. The gate is called: (1) immediately after **Connect** succeeds (per side); (2) at the start of **LoadSettingsAsync** (FittingViewModel) before any ReadParameters; (3) before **SaveCurrentMemoryAsync** / **SaveAllMemoriesAsync** (FittingViewModel) and before **SessionEndService.RunSaveAllOnDispatcherAsync**. If **IsConfigured** is false, the UI shows "Connected | Not configured", disables Save to Device, and shows a banner with **LastConfigError** (e.g. "Device must be configured (InitializeDevice returned false)."). Logs use the prefix `[EnsureInit]` and are written to Debug and ScanDiagnostics so the exact step that failed is visible.
...
- `src/App/Services/SessionEndService.cs`: **RunSaveAllOnDispatcherAsync** (guard before **BurnMemoryToNvmAsync** + **VerifyMemoryMatchesNvmAsync**).
...
13. **Graphs panel (Fitting page):** Sound Designer–style graphs sidebar:
...
- **\[Perf\] logging:** Stopwatch timings are written to Debug output: BuildTabHeaders, LoadTab, LoadGroup, ReadSpace (per side), RenderRows, SaveToDevice (with dirty count), **EnsureInit** (probe ms). Use these to find slow operations. **Where time goes (typical from logs):** (1) **Discovery** — Left port ~5–6 s, Right port ~3–4 s, total DetectBoth ~9 s; driven by SDK/HI-PRO hardware, not app code. (2) **EnsureInitializedAndConfigured** — ~400–800 ms when "already configured" because the probe runs a full **ReadParameters(kActiveMemory)** to detect E_UNCONFIGURED_DEVICE. (3) **Read memory from device** — ~2.4–2.6 s per memory (kActiveMemory + kSystemActiveMemory). (4) **Save (write)** — ~2.6 s + ~1.5 s per memory (~4 s total). Search Debug output for `[Perf]` or `duration=` or `ms=` to see timings.
...
11. **Fast loading (single-memory snapshot):** When reading from a connected device, `SoundDesignerService.ReadMemorySnapshotAsync` selects a memory context and builds the parameter list with **BuildSnapshotForMemory(product, selectedMemoryIndex, side)** instead of **BuildFullSnapshot**. That enumerates only one memory (~595 parameters) instead of all 8 memories (~4760), which:
...
3. **SdkLifecycle state:** A global **SdkLifecycle** state (`Uninitialized` → `Initializing` → `Ready` → `Disposing` → `Disposed`) prevents re-initialization during teardown. **ConnectDevicesView** does not call SDK init when **HasActiveSession** or **SdkLifecycle.IsDisposingOrDisposed**. **SdkManager.Initialize** / **ReloadForFirmware** throw if lifecycle is Disposing/Disposed. After **ClearSessionAsync** completes, **SdkLifecycle** is reset to **Uninitialized** so the headset can be used again (discovery and Connect/ReloadForFirmware succeed on the next session).

4. **Safe session teardown (ClearSessionAsync):** End Session uses **DeviceSessionService.ClearSessionAsync()**: (A) Stop Live Mode, (B) Cancel in-flight operations (session CTS), (C) **SdkGate.BeginDispose()** then **await SdkGate.DrainAsync()**, (D) **SdkGate.RunCleanupToDispose** to run **ConnectionService.Cleanup()** and **SdkManager.Dispose()** on the gate, (E) clear references, **SdkGate.ResetForNewSession()**, and **SdkLifecycle.SetState(Uninitialized)** so the next session can initialize and connect. **SessionEndService** awaits **ClearSessionAsync()** before navigating or restarting discovery—no use-after-free.
...
- **SoundDesigner SDK — documentation rule:** All SDK-related behavior (NVM Restore/Burn, Memory switching, Configure/Initialize, session Save and End) must be justified by the **local vendor docs** at  
  ...
  See **`.cursor/rules/rules.mdc`** §0: code must reference doc filename + section; undocumented APIs must not be implemented (log "Not supported by docs" and keep feature disabled).
- **Persistence across disconnect** requires writing to **NVM** (e.g. `WriteParameters(memoryIndex)` or `ParameterSpace.kNvmMemoryN`), not only to RAM (`kActiveMemory`). The app discovers the NVM write API at runtime via reflection (ParameterSpace values and `BeginWriteParameters` overloads). If the .NET SDK does not expose an NVM write, the app logs **"Not supported by docs: no NVM write API found; persistence may fail after disconnect."** and saving may not persist after the user disconnects. See **SoundDesignerService** (discovery and NVM write in `WriteMemorySnapshotAsync`) and plan **fix_nvm_persistence_on_reconnect_2e09772b.plan.md**.
