...
**Memory synchronization:** Audiogram and Fitting share **DeviceSessionService.SelectedMemoryIndex** as the single source of truth when a device is connected. AudiogramViewModel subscribes to `session.PropertyChanged` for `SelectedMemoryIndex` and syncs its local state. FittingViewModel syncs from session at the start of **LoadSettingsAsync** and subscribes to `session.PropertyChanged` for `SelectedMemoryIndex` so that when memory is changed in Audiogram, Fitting updates immediately (and vice versa). Navigation between screens preserves the same active memory.
